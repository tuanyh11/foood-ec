...
import {RiCake3Line, RiShoppingBasketLine, RiUser3Line, RiMenuLine, RiCloseFill} from 'react-icons/ri'
    ...
    <div className='fixed top-0 right-0 left-0 shadow-sm bg-white z-10' >
      <Container className='w-full'>
        ...
        <div className='flex items-center p-[10px_0] h-[100px] justify-between'>
            ...
            {/* nav */}
            <nav className={`fixed right-0 top-0 left-0 bg-rgba_1 bottom-0 w-full 
            z-10  ${isClose ? 'translate-x-[-100%]': 'translate-x-[0]'} transition-all lg:hidden md:hidden`}>
              <div>
              ...
              <div className='p-[20px_10px] flex flex-col items-center h-full justify-center'>
                {navs.map((item, index) => (
                  ...
                  >
                    <span className="capitalize p-[10px_0] text-cyan-50 transition-all block font-semibold text-xl hover:text-main mb-2">{item.name}</span>
                  </NavLink>
                ))}
              </div>
            </nav>
            ...
            <nav className={`hidden lg:block md:block`}>
              <div className='flex gap-5'>
                {navs.map((item, index) => (
                  ...
                  >
                    {({isActive}) => 
                      <span className={`capitalize  transition-all block font-semibold text-[20px] hover:text-main ${isActive ? 'text-main': ''}`}>{item.name}</span>
                    }
                  </NavLink>
                ))}
              </div>
            ...
            <div className="flex items-center gap-2 text-slate-700">
              <div className="relative p-[10px] cursor-pointer" onClick={() =>setOpenCart(true)}>
                <RiShoppingBasketLine className="w-5 h-5  text-slate-700 " />
                <p className="absolute top-[8px] right-[5px] w-[15px] 
                h-[15px] rounded-[50%] bg-green-500 text-[10px] text-center !flex items-center justify-center text-white text-limit">{cart.products.length}</p>
              </div>
